async function openTravelDone() {
  // refs
  const listEl = document.getElementById("travelListDone");
  const countEl = document.getElementById("countTravelsDone");
  const fltUnit = document.getElementById("fltUnitDone");
  const fltDate = document.getElementById("fltDateDone");
  const fltService = document.getElementById("fltServiceDone");
  const btnApply = document.getElementById("btnApplyDone");
  ...
  function injectStylesOnce() {
    // ✅ reaproveita o MESMO id => mesmo layout dos pendentes
  ...
  async function fetchTravels() {
    const payload = {
      status: "REALIZADA", // ✅ concluídas
      unitKey: (fltUnit?.value || "").trim(),
      tripDate: (fltDate?.value || "").trim(), // ✅ já começa com hoje
      ...
      listEl.innerHTML = `
        <div class="col-12">
          <div class="text-muted small">
            Nenhuma viagem concluída encontrada com esses filtros.
          </div>
        </div>
      `;
      return;
    ...
    travels.forEach((t) => {
      ...
      const statusBadge = `
        <span class="travel-pill bg-success text-white">
          <i class="bi bi-check2-circle"></i> ${safeText(t?.status || "REALIZADA")}
        </span>
      `;

      // ✅ informativo: sem botões
      col.innerHTML = `
  ...
  btnClear &&
    (btnClear.onclick = async () => {
      ...
      if (fltDate) fltDate.value = todayISO; // ✅ limpa => volta pro dia atual
