async function openTravelHistory() {
  const listEl = document.getElementById("travelListHist");
  const countEl = document.getElementById("countTravelsHist");
  const fltUnit = document.getElementById("fltUnitHist");
  const fltDate = document.getElementById("fltDateHist");
  const fltService = document.getElementById("fltServiceHist");
  const fltStatus = document.getElementById("fltStatusHist");
  const btnApply = document.getElementById("btnApplyHist");
  const btnClear = document.getElementById("btnClearHist");

  // ✅ começa SEMPRE com o dia atual (mesma pegada do seu dashboard)
  ...
  function statusBadgeHtml(status) {
    ...
    if (st === "REALIZADA") {
      return `
        <span class="travel-pill bg-success text-white">
    ...
    if (st === "CANCELADA") {
      return `
        <span class="travel-pill bg-danger text-white">
    ...
    return `
      <span class="travel-pill bg-warning text-dark">
        <i class="bi bi-hourglass-split"></i> ${safeText(st || "PENDENTE")}
      </span>
    `;
  }

  function injectStylesOnce() {
    // ✅ mesmo estilo dos cards (id igual ao que você já usa)
    ...
    style.textContent = `
      ...
      .travel-actions .btn{ border-radius: 12px; }
      .travel-title{
        font-weight: 900;
        font-size: 1.05rem;
        line-height: 1.15;
  ...
  // ✅ busca geral:
  // - se status vier vazio (Todos), busca 3 vezes (PENDENTE/REALIZADA/CANCELADA) e junta
  async function fetchTravels() {
    const unitKey = (fltUnit?.value || "").trim();
    const tripDate = (fltDate?.value || "").trim();
    const serviceKey = (fltService?.value || "").trim();
    const status = (fltStatus?.value || "").trim(); // "" = todos

    const basePayload = { unitKey, tripDate, serviceKey };

    let rows = [];

    if (status) {
      const resp = await fetchData("/transportPhase/list", "PUT", {
        ...basePayload,
        status,
      });
      rows = Array.isArray(resp?.travels) ? resp.travels : [];
    } else {
      const statuses = ["PENDENTE", "REALIZADA", "CANCELADA"];
      const results = await Promise.all(
        statuses.map((st) =>
          fetchData("/transportPhase/list", "PUT", {
            ...basePayload,
            status: st,
          }).catch(() => ({ travels: [] })),
        ),
      ...
      rows = results.flatMap((r) =>
        Array.isArray(r?.travels) ? r.travels : [],
      );
    }

    // ✅ remove duplicados por _key (caso teu backend retorne repetido)
    const seen = new Set();
    travels = rows.filter((t) => {
      const k = t?._key;
      if (!k) return false;
    ...
    // ✅ ordena por updatedAt/createdAt desc (se existir), senão por tripDate desc
    travels.sort((a, b) => {
  ...
  function render() {
    ...
    travels.forEach((t) => {
      ...
      const startLabel = firstStartLabel(t);
      ...
      const badge = statusBadgeHtml(st);

      // ✅ ações só para pendente (mantém pegada)
      const actionsHtml =
        st === "PENDENTE"
          ? `
    ...
    listEl.querySelectorAll("button[data-action]").forEach((btn) => {
      btn.onclick = async () => {
        ...
        if (action === "done") {
          btn.disabled = true;
          try {
            await fetchData("/transportPhase/mark-done", "PUT", {
              travelKey: key,
            });
            await fetchTravels();
            render();
        ...
        if (action === "cancel") {
          ...
          btn.disabled = true;
          try {
            await fetchData("/transportPhase/travel/cancel", "PUT", {
              travelKey: key,
              reason,
            });
            await fetchTravels();
